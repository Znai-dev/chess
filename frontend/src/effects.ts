...
export const KIND_STYLE: Record<MoveKind, { empty: CSSProperties; capture: CSSProperties; label: string }> = {
  ...
  break:    { label: '⚒',
              empty:   { background: 'radial-gradient(circle, rgba(251,146,60,.7) 26%, transparent 30%)' },
              capture: { background: 'radial-gradient(circle, transparent 60%, rgba(251,146,60,.9) 62%)' } },
  teleport: { label: '🌀',
